...
## 009: Polish の raw フォールバックは継続し、適用結果だけ非ブロッキングに表示する

**決定**: Polish API が失敗・空応答・不正応答になった場合も raw テキストの注入は継続する。一方で、履歴と完了表示では注入成功とは別に `Polish適用`、`Polish・変更なし`、`Rawフォールバック` を区別する。

**理由**:
従来は raw フォールバック後も履歴が `Polish / 成功` とだけ表示され、モデルが同じ本文を返したのか、API が失敗したのかを区別できなかった。音声入力を止めない方針を維持しながら、Polish の品質問題と通信問題を安全に切り分けられる必要がある。

**実装方針**:
- 注入の成功・失敗と Polish の適用状態を別フィールドで管理する。
- API エラー本文、transcript、カスタム指示は履歴へ保存しない。
- 既存履歴は適用状態不明として読み込み、誤ってフォールバック扱いしない。
- フォールバック表示は入力を止めるモーダルではなく、履歴と完了表示の小さな状態ラベルに限定する。
