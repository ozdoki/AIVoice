...
fn estimate_words(text: &str) -> u32 {
    let whitespace_words = text.split_whitespace().filter(|part| !part.is_empty()).count();
...
pub fn record_usage(app: &AppHandle, metrics: SessionMetrics) -> anyhow::Result<Vec<UsageDaySummary>> {
    ...
    summary.words = summary.words.saturating_add(estimate_words(&metrics.final_text));
...
mod tests {
    ...
    fn dictionary_word_normalization_rejects_empty() {
        assert_eq!(normalize_dictionary_word("  Obsidian  "), Some("Obsidian".to_string()));
