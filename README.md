...
- グローバルホットキー
  ...
  - Raw / Polish モード切替
  - 選択テキストからの明示的な修正学習
  - 選択テキストを音声指示で編集（既定 `Ctrl + Shift + F9`）
- Windows 音声入力
  - マイク選択
  - WASAPI 録音
  - 録音レベル表示
...
- 履歴・復元
  ...
  - 失敗・中断した録音の復元候補
- 選択音声編集
  - 外部アプリの選択本文を取得してから編集指示を録音
  - 原文・音声指示・編集案を確認する必須プレビュー
  - 同じウィンドウ・プロセス・選択本文を再確認できた場合だけ置換
  - 安全に再確認できないアプリでは編集案のCopyのみ
- UI
  - メイン画面
  - フローティングバー
  - 初回セットアップ
...
- F9の選択音声編集は例外として、ユーザーが明示的に選択した原文と音声から認識した編集指示を設定中の外部APIへ送信します。原文は履歴・Recoveryへ保存せず、保留中のプレビューは10分で破棄します。
- 設定画面の「データ処理経路」には、保存済み設定から導いたAPI送信先ホスト、モデル、送信され得るデータ、ローカル保存項目を表示します。これはローカル処理と外部API処理を切り替える機能ではありません。
